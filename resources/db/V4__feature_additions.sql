...
CREATE INDEX idx_shop_holidays_shop_date ON shop_holidays(shop_id, date);
...
CREATE INDEX idx_announcements_shop ON announcements(shop_id);
CREATE INDEX idx_announcements_active ON announcements(shop_id, valid_from, valid_to);
...
CREATE UNIQUE INDEX idx_reviews_user_token       ON reviews(user_id, token_id)       WHERE token_id IS NOT NULL;
CREATE UNIQUE INDEX idx_reviews_user_appointment ON reviews(user_id, appointment_id) WHERE appointment_id IS NOT NULL;
...
CREATE UNIQUE INDEX idx_waitlist_active_user_shop ON waitlist(shop_id, user_id)
    WHERE status = 'WAITING';

CREATE INDEX idx_waitlist_shop_status ON waitlist(shop_id, status, joined_at);
